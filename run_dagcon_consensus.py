...
if __name__ == "__main__":
    ...
    files = commands.getoutput("find {} -iname \"*fastq\"".format(wkdir)).split()
    if len(files) == 0: ## likely that fastq are compressed
        files = commands.getoutput("find {} -iname \"*fastq.gz\"".format(wkdir)).split()
        gz = "on"    
    else:
        gz = "off"
    fastq_dic = {}
        ...
        while fastqline < len(lines):
            if "@" in lines[fastqline]:	# bypass nasty bug in which fastq's are corrupted. Corrupted will be skipped, not fixed
                ...
                if str(f) not in line_dic:
                    line_dic[str(f)] = [{read:[fastqline + 1, fastqline + 4]}]
                else:
                    line_dic[str(f)] += [{read:[fastqline + 1, fastqline + 4]}]
                fastqline += 4
            else:
    ...
    for item in line_dic:
        ...
        for f in line_dic[item]:
            ...
            if readf == "pass": 
                ...
                write_file.write("{cat} {item} | sed -n \'{pos0},{pos1}\'p | {lastall} -Q 1 -p {lastparam} {refgenome_target_db} -P 1 /dev/stdin | {lastsplit} | {mafconvert} -f {refgenome_target_db}.dict sam -r \"ID:{fastq} PL:nanopore SM:{fastq}\" /dev/stdin | {sambamba} view -S -f bam /dev/stdin | {sambamba} sort -t 1 /dev/stdin -o {outdir}/bam/{folder}_{subnumber}/{fastq}.sorted.bam\n ".format(
                        cat = cat, 
                        item = item,
                        pos0 = position[0],
                        pos1 = position[1],
            ...
            if count == (number - 1):
                job_list += [outdir + "/x" + folder + "_job_" + str(subnumber) + ".sh"]
                write_file.close()
                subnumber += 1
        ...
        job_list += ["{0}/x{1}_job_{2}.sh".format(outdir, folder, subnumber)]
        write_file.close()

    os.mkdir(outdir + "/SH")
    job_id = []
    for job in job_list:
        action = "qsub -q all.q -P {project} -l h_rt={timeslot} -l h_vmem={max_mem}G -R y -cwd -pe threaded {threads} {job} -o {outdir}/SH/ -e {outdir}/SH/ -m a -M {mail}".format(
            project = project,
            timeslot = timeslot,
            max_mem = max_mem,
            threads = threads,
    ...
    write_file = open("{}/full_target_mapping.sh".format(outdir), "w")
    ...
    action = ("qsub -cwd -q all.q -P {project} -l h_rt=36:00:00 -l h_vmem=40G -hold_jid {joinedjob} {outdir}/full_target_mapping.sh -o {outdir}/SH/ -e {outdir}/SH/ -m ae -M {mail}".format(
        project = project,
        joinedjob = ",".join(job_id),
        outdir = outdir,
        mail = opt.mail
        ))
    ...
    write_file = open("{}/cleanup.sh".format(outdir), "w")
    write_file.write("mv {outdir}/*sh* {outdir}/SH\n".format(outdir = outdir)) 
    write_file.write("cd {}/SH\n".format(outdir)) 
    write_file.write("zip -m SH.zip *\n")
    write_file.close()

    action = ("qsub -cwd -q all.q -P {project} -l h_rt=1:0:00 -l h_vmem=1G -hold_jid {joinedjob} {outdir}/cleanup.sh -o {outdir}/SH/ -e {outdir}/SH/ -m baes -M {mail}".format(
        project = project,
        joinedjob = ",".join(hold_id),
        outdir = outdir,
        mail = opt.mail
        ))

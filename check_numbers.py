...
files = subprocess.getoutput("find "+rawdir+ " -iname \"*fastq\"").split()

if len(files) == 0: ## likely that fastq are compressed
    fastq = subprocess.getoutput("find "+rawdir+ " -iname \"*fastq.gz\" -exec gunzip -cd {} \; | wc -l")
else:
    fastq = subprocess.getoutput("find "+rawdir+ " -iname \"*fastq\" -exec cat {} \; | wc -l")

reads = int(int(fastq)/4)
bams = subprocess.getoutput("find "+prodir+"/bam/ -iname \"*tar\" -exec tar -tf {} \; |grep bai | wc -l")
m5 = subprocess.getoutput("find "+prodir+"/m5/ -iname \"*tar\" -exec tar -tf {} \; | wc -l")
consensus = subprocess.getoutput("find "+prodir+"/consensus/ -iname \"*tar\" -exec tar -tf {} \; | wc -l")
